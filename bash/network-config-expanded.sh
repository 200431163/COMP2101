...
tinterface=$(lshw -class network | awk '/logical name:/{print $3}' | wc -l)
#Each step of activity is processed into format of an array by generating a loop
for((w=1;w<=$tinterface;w+=1));
do
  interface=$(lshw -class network |
    awk '/logical name:/{print $3}' | awk -v z=$w 'NR==z{print $1; exit}')
  if [[ $interface = lo* ]] ; then continue ; fi
  #Make conditions to work for the skipping of loopback interface
  ipv4_address=$(ip a s $interface | awk -F '[/ ]+' '/inet /{print $3}')
  ipv4_hostname=$(getent hosts $ipv4_address | awk '{print $2}')
  network_address=$(ip route list dev $interface scope link|cut -d ' ' -f 1)
  network_number=$(cut -d / -f 1 <<<"$network_address")
  network_name=$(getent networks $network_number|awk '{print $1}')
  #Wrinting for getting a command output in the form of variables in simple lines
  ...
  #Using echo command to print all information
done
#To exit the loop
#####
