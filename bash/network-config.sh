...
myhostname=$(hostname) # getting hostname
interface_name=$(ip a |awk '/: e/{gsub(/:/,"");print $2}') #Getting a name for interface
lan_address=$(ip a s $interface_name|awk '/inet /{gsub(/\/.*/,"");print $2}') #Knowing lan address
lan_hostname=$(getent hosts $(ip a s $interface_name)|awk '/inet /{gsub(/\/.*/,"");print $2}' | awk '{print $2}') #9. To find hostname of lan alongwith variable and getent hosts
external_ip=$(curl -s icanhazip.com) #With the help of curl, getting ip address from external source
external_name=$(getent hosts $external_ip| awk '{print $2}') #While looking host files, getting name for external name 
router_address=$(ip r s default| cut -d ' ' -f 3) #Acquiring router address 
router_hostname=$(getent hosts $router_address|awk '{print $2}') #To find router hostname using getent hosts
